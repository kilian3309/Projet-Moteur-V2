...
void CreateLoadingScreen(HINSTANCE hInst = (HINSTANCE)nullptr, int maxRange=6) {
	...
	hwndPB = CreateWindowEx(0, PROGRESS_CLASS, (LPCTSTR)NULL, WS_CHILD | WS_VISIBLE | PBS_SMOOTH, (width-600)/2, height/5, 600, 30, hwndLS, 0, hInst, NULL);
...
void IncrementLoading() {
	...
	MessageBoxK(L"", to_wstring(SendMessage(hwndPB, PBM_GETPOS, 0, 0)) + to_wstring(SendMessage(hwndLS, PBM_GETRANGE, 0, 0)));
